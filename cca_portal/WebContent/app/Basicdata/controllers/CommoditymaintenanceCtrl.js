...
angular.module('app.Basicdata').controller('CommoditymaintenanceCtrl', function ($scope,$state,$rootScope,$stateParams,$location,APP_CONFIG,CommoditymaintenanceService,Upload) {

    //加载页面、搜索功能
    $scope.CommoditySel = '';
    $scope.SubcommoditySel='';
    $scope.goPage = function(){
        $scope.page = {
            limit : APP_CONFIG.limit,
            page : APP_CONFIG.page,
            beanName : $scope.CommoditySel,
            beancName:$scope.SubcommoditySel
        };
        console.log($scope.page);
        CommoditymaintenanceService.getPage($scope.page).then(function(data){
            if(data.code == 0){
    ...
    $scope.del = function(){
        if($scope.arr.length<=0){
            ...
            if(confirm('确认要删除？')){
                ...
                CommoditymaintenanceService.delList($scope.arrlist).then(function(data){
                    if(data.code == 0){
                        alert('删除成功！');
                        $scope.goPage();
                    }else {
    ...
    $scope.downLoad = function(){
        $scope.load = {
            type : 'commoditumaintence'
        }
        CommoditymaintenanceService.download($scope.load).then(function(data){
            console.log(data);
    ...
    $scope.upload = function(){
        Upload.upload({
            //服务端接收
            url:APP_CONFIG.baseUrl+ '/api/cmt/attachments',
            data : {
                file : $scope.myfiles,
